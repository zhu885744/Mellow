    ...
    <section class="card card-pad panel">
      <header class="panel-head">
        ...
        <div class="head-actions">
          ...
          <router-link to="/manage/moments" class="btn btn-ghost btn-sm">
            <i class="bi bi-person-lines-fill" /> 我的动态
          </router-link>
          <router-link to="/moments" class="btn btn-primary btn-sm">
            <i class="bi bi-plus-lg" /> 发布动态
          </router-link>
        </div>
      </header>
 ...
 * 只负责两件事：
 * 1. 拉取各状态计数并渲染统计卡片；
 * 2. 通过 provide 把「统计卡片 ←→ 列表筛选」的联动能力交给子路由 MomentList.vue。
 */
import { ref, computed, provide, onMounted, onUnmounted } from 'vue'
import { call } from '@/api/request'
import { MOMENT_FILTERS, momentCountParams } from '@/utils/moment'

// 与列表筛选共用 MOMENT_FILTERS（all/pub/pending/reject/draft/trash），统计口径不会各写一份
const stats = ref(MOMENT_FILTERS.map((f) => ({ ...f, value: 0, error: '' })))
...
async function loadCounts() {
  ...
  const results = await Promise.all(
    MOMENT_FILTERS.map(async (f) => {
      try {
        // 管理员（permit）不会被追加 audit=1，因此可以统计到全部状态
        const res = await call('moments', 'count', { method: 'GET', params: momentCountParams(f.key) })
...
.stat-grid {
  display: grid;
  /* 卡片数量随 MOMENT_FILTERS 增减，用自适应列数，避免每加一个筛选就要改样式 */
